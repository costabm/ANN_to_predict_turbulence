...
# =============================================================================
                    ...
                    def Coh_lateral_u(f,Cy):
                        return np.exp(-1*np.divide(f,U_coh)*np.power(np.power(3*dx,2)+np.power(Cy*dy,2),0.5))*np.cos(dx*2*np.pi*f/U_coh) 
                    def Coh_lateral_v(f,Cy):
                        return np.exp(-1*np.divide(f,U_coh)*np.power(np.power(3*dx,2)+np.power(Cy*dy,2),0.5))*np.cos(dx*2*np.pi*f/U_coh) 
                    def Coh_lateral_w(f,Cy):
                        return np.exp(-1*np.divide(f,U_coh)*np.power(np.power(3*dx,2)+np.power(Cy*dy,2),0.5))*np.cos(dx*2*np.pi*f/U_coh) 
                    ...
                    [Cuy[flag_case,0]],  pcov = curve_fit(Coh_lateral_u,f_s[indfu_s[0]:indfu_e[0],0], Coh_u_s[indfu_s[0]:indfu_e[0],0],p0=(10),maxfev=100000)
                    [Cvy[flag_case,0]],  pcov = curve_fit(Coh_lateral_v,f_s[indfu_s[0]:indfu_e[0],0], Coh_v_s[indfu_s[0]:indfu_e[0],0],p0=(6.5),maxfev=100000)
                    [Cwy[flag_case,0]],  pcov = curve_fit(Coh_lateral_w,f_s[indfu_s[0]:indfu_e[0],0], Coh_w_s[indfu_s[0]:indfu_e[0],0],p0=(6.5),maxfev=100000)
                ...
                #%%
                    ...
                    ax1.semilogx(f,Coh_lateral_u(f,Cuy[flag_case,0]),  'r-', label='$Fitted$ (Cux='+ str(np.round(Cux[flag_case,0],2))+';Cuy='+str(np.round(Cuy[flag_case,0],2))+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.semilogx(f,Coh_lateral_u(f,10),  'b--', label='$Ref$ (Cux='+ str(3)+';Cuy='+str(10)+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6) 
                    ...
                    ax1.semilogx(f,Coh_lateral_v(f,Cvy[flag_case,0]),  'r-', label='$Fitted$ (Cvx='+ str(np.round(Cvx[flag_case,0],2))+';Cvy='+str(np.round(Cvy[flag_case,0],2))+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.semilogx(f,Coh_lateral_v(f,6.5),  'b--', label='$Ref$ (Cvx='+ str(3)+';Cvy='+str(6.5)+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.fill_between(f,-10,10,where= f>f_hor,facecolor='grey', alpha=0.5)   
                    ax1.set_xlim(0.005, 2)
                    ...
                    ax1.semilogx(f,Coh_lateral_w(f,Cwy[flag_case,0]),  'r-', label='$Fitted$ (Cwx='+ str(np.round(Cwx[flag_case,0],2))+';Cwy='+str(np.round(Cwy[flag_case,0],2))+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.semilogx(f,Coh_lateral_w(f,6.5),  'b--', label='$Ref$ (Cwx='+ str(3)+';Cwy='+str(6.5)+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.fill_between(f,-10,10,where= f>f_hor,facecolor='grey', alpha=0.5)   
                    ax1.set_xlim(0.005, 2)
