    ...
    #%%
        ...
        for flag_case in range(0,num_case):
            ...
            if  nan_ratio < 0.05:
                ...
                if delta_U[0,0]<0.3*meanU[0,0] and delta_Dir[0,0]<20 and dy<=1.5:    # this is to check the stationarity range of moving mean should be smaller than +-10% of the static mean      
...
# =============================================================================
                    ...
                    def Coh_lateral_dx(f,Cx):
                        return np.exp(-1*np.divide(f,U_coh)*Cx*dx)*np.cos(dx*2*np.pi*f/U_coh) 
                    ...
                    Cux[flag_case,0],  pcov = curve_fit(Coh_lateral_dx,f_s[indfu_s[0]:indfu_e[0],0], Coh_u_s[indfu_s[0]:indfu_e[0],0],p0=(0.5),maxfev=100000)
                    Cvx[flag_case,0],  pcov = curve_fit(Coh_lateral_dx,f_s[indfu_s[0]:indfu_e[0],0], Coh_v_s[indfu_s[0]:indfu_e[0],0],p0=(0.5),maxfev=100000)
                    Cwx[flag_case,0],  pcov = curve_fit(Coh_lateral_dx,f_s[indfu_s[0]:indfu_e[0],0], Coh_w_s[indfu_s[0]:indfu_e[0],0],p0=(0.5),maxfev=100000)
                ...
                #%%
                    ...
                    save_tite = 'Storm No.' + str(event+1) + '_coherence fitting Osp2_'\
                                 + str(date_begin.year) + '_' + str(date_begin.month) + '_' + str(date_begin.day)\
                                     + '_' + str(date_begin.hour) + 'smooth pure_dx.png'
        ...
        with open(path + save_tite + 'pure_dx_smooth_json.txt', "w") as outfile:
