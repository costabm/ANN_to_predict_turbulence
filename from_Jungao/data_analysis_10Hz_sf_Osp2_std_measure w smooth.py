...
for event in range(0,cases['Time_storm'].size):
    ...
    # Osp2 
    info_osp2        = pd.read_excel('D:/DATA/files_osp2.xls', sheet_name='Height')  
    
    if os.path.isfile('D:/DATA/osp2/' +'Osp2_'+ str(time.year) +'_' + str(time.month)+'_' + str(time.day)+ '_storm_raw_data.pkl'):
        with open('D:/DATA/osp2/' +'Osp2_'+ str(time.year) +'_' + str(time.month)+'_' + str(time.day)+ '_storm_' + 'Osp2_1h' +'_ins.pkl', 'rb') as f:    
            Osp2_1h_ins  = pickle.load( f )   
        with open('D:/DATA/osp2/' +'Osp2_'+ str(time.year) +'_' + str(time.month)+'_' + str(time.day)+ '_storm_' + 'Osp2_1h' +'_mean.pkl', 'rb') as f:    
            Osp2_1h_mean = pickle.load( f )  



        cases_osp2= pd.read_excel('files_osp2.xls', sheet_name='files_osp2')
        ...
        for flag_case in range(0,num_case):
            ...
            date_new_start =  Osp2_1h_mean.iloc[0,0]  - delta_30min          
            date_new_end   =  Osp2_1h_mean.iloc[-1,0] + delta_30min  -  delta_ms 
            ...
            Num       = Osp2_1h_mean.index[Osp2_1h_mean['Time'].dt.tz_localize(None) == date].tolist()        
            ...
            nan_ratio = Osp2_1h_ins.loc[num_s[0]:num_e[0],['A_u','B_u','C_u']].isnull().sum().sum()/Osp2_1h_ins.loc[num_s[0]:num_e[0],['A_u','B_u','C_u']].size.sum()
            if  nan_ratio < 0.05:
                
                # get the raw data (interpolate the data for NAN using linear method)
                
                dd    = np.asarray(Osp2_1h_ins.loc[num_s[0]:num_e[0]-1,['A_dir','B_dir','C_dir']].interpolate(axis=0,limit_direction='both'))
                UU    = np.asarray(Osp2_1h_ins.loc[num_s[0]:num_e[0]-1,['A_u','B_u','C_u']].interpolate(axis=0,limit_direction='both'))        
                VV    = np.asarray(Osp2_1h_ins.loc[num_s[0]:num_e[0]-1,['A_v','B_v','C_v']].interpolate(axis=0,limit_direction='both'))  
                WW    = np.asarray(Osp2_1h_ins.loc[num_s[0]:num_e[0]-1,['A_w','B_w','C_w']].interpolate(axis=0,limit_direction='both'))  
                ...
                # reference data 
                z0       = 0.01;
                Lux_ref  = 100*np.power((np.asarray(info_osp2.iloc[0,:])/10),0.3);
                Lvx_ref  = 1/4* Lux_ref;
                ...
                Iu_ref   = (1./np.log(np.asarray(info_osp2.iloc[0,:])/z0)).reshape((1,np.size(Lux_ref)))
                Iv_ref   = 3/4.*Iu_ref;
                Iw_ref   = 1/2.*Iu_ref;    
    ...
    #             ax2.plot(mdates.num2date(tt), UU[:,0], 'b-',label='$\overline{U} + u $ $(Osp2\_A)$',markeredgecolor='k',markersize=8,alpha=0.5)
    #             ax2.plot(mdates.num2date(tt), movingU[:,0], 'r-',label='$\overline{U} $ $(Osp2\_A)$',markeredgecolor='k',markersize=8,alpha=0.5)
    #             ax2     = plt.subplot(312)
    #             locator = mdates.AutoDateLocator(minticks=7, maxticks=9)
    #             formatter = mdates.ConciseDateFormatter(locator)
    #             ax2.xaxis.set_major_locator(locator)
    #             ax2.xaxis.set_major_formatter(formatter)
    #             ax2.plot(mdates.num2date(tt), UU[:,1], 'b-',label='$\overline{U} + u $ $(Osp2\_B)$',markeredgecolor='k',markersize=8,alpha=0.5)
    #             ax2.plot(mdates.num2date(tt), movingU[:,1], 'r-',label='$\overline{U} $ $(Osp2\_B)$',markeredgecolor='k',markersize=8,alpha=0.5)
    #             ax2     = plt.subplot(313)
    #             locator = mdates.AutoDateLocator(minticks=7, maxticks=9)
    #             formatter = mdates.ConciseDateFormatter(locator)
    #             ax2.xaxis.set_major_locator(locator)
    #             ax2.xaxis.set_major_formatter(formatter)
    #             ax2.plot(mdates.num2date(tt), UU[:,2], 'b-',label='$\overline{U} + u $ $(Osp2\_C)$',markeredgecolor='k',markersize=8,alpha=0.5)
    #             ax2.plot(mdates.num2date(tt), movingU[:,2], 'r-',label='$\overline{U} $ $(Osp2\_C)$',markeredgecolor='k',markersize=8,alpha=0.5)            
    # =============================================================================
                #%
                ...
                for i in range(0, 3):
                    if delta_U[0,i]<0.3*meanU[0,i] and delta_Dir[0,i]<20:    # this is to check the stationarity range of moving mean should be smaller than +-10% of the static mean      
                        ...
                        f_ver      = 1/6.89*0.9
                        matplotlib.use('Agg')                        
                        Uref       =  meanU[0,i]*(1+np.log(np.asarray(info_osp2.iloc[0,:])[i]/10)/np.log(10/z0))
                        Su_n_froya =  Sfn_from_Sf(f_s,Froya_u(f_s[:,0], Uref, np.asarray(info_osp2.iloc[0,:])[i]).reshape(np.size(f_s),1))
                        plt.close("all")
                        fig     = plt.figure(figsize=(20, 12))
                        ax1     = plt.subplot(321)
                        ...
                        ax1.semilogx(f[:,0],Froya_u(f[:,0], Uref, np.asarray(info_osp2.iloc[0,:])[i]),  'c--',label='$Frøya$ $ ref$',linewidth=2, markeredgecolor='k',markersize=8)           
                        ...
                        fig.suptitle('Storm No.' + str(event+1)+ ' Dir=' + str(np.round(meanDir[0,i],1)) + '$^o$'+'  Osp2 at height ' + str(np.round(np.asarray(info_osp2.iloc[0,:])[i],1)) + 'm ' \
                        ...
                        path      = 'D:/DATA/Plots/Osp2 Spectra fitting/'
                        save_tite = 'Storm No.' + str(event+1) + '_spectrum fitting Osp2 sensor No.' + str(i+1) + '_'\
                        ...
                        fig.suptitle('Storm No.' + str(event+1) + ' Osp2 at height ' + str(np.round(np.asarray(info_osp2.iloc[0,:])[i],1)) + 'm '\
                                     + 'from '  + date_begin.strftime("%Y-%m-%d %H:%M") + ' to ' + date_stop.strftime("%Y-%m-%d %H:%M"), fontsize=25)
                        plt.rc('xtick', direction='in', color='k')
                        plt.rc('ytick', direction='in', color='k')
                        g1 = plt.grid(b=True, which='major', color='k', linestyle='-', linewidth=0.35)
                        ...
                        path      = 'D:/DATA/Plots/Osp2 Spectra fitting/'
                        save_tite = 'Storm No.' + str(event+1) + '_time series and spectra Osp2 sensor No.' + str(i+1) + '_'\
                                     + str(date_begin.year) + '_' + str(date_begin.month) + '_' + str(date_begin.day)\
        ...
        path      = 'D:/DATA/Results/Osp2 Spectra fitting/'
        
        save_tite = 'Storm No.' + str(event+1) + '_spectrum fitting Osp2 sensor No.' + str(3) + '_'\
