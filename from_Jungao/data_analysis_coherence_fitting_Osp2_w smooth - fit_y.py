...
for event in range(0,cases['Time_storm'].size):
    ...
    # Osp2 height info
    info_osp2        = pd.read_excel('E:/DATA/files_osp2.xls', sheet_name='Height')  
    
    if os.path.isfile('E:/DATA/osp2/' +'Osp2_'+ str(time.year) +'_' + str(time.month)+'_' + str(time.day)+ '_storm_raw_data.pkl'):
        with open('E:/DATA/osp2/' +'Osp2_'+ str(time.year) +'_' + str(time.month)+'_' + str(time.day)+ '_storm_' + 'Osp2_1h' +'_ins.pkl', 'rb') as f:    
            Osp2_1h_ins  = pickle.load( f )   
        with open('E:/DATA/osp2/' +'Osp2_'+ str(time.year) +'_' + str(time.month)+'_' + str(time.day)+ '_storm_' + 'Osp2_1h' +'_mean.pkl', 'rb') as f:    
    ...
    #%%
        ...
        for flag_case in range(0,num_case):
            ...
            if  nan_ratio < 0.05:
                ...
                if delta_U[0,0]<0.3*meanU[0,0] and delta_Dir[0,0]<20:    # this is to check the stationarity range of moving mean should be smaller than +-10% of the static mean      
                # spectral analysis of the wind time series
                    ...
                    yaw_s = -8   # for ospøya2, the boom is -8 degress from true north or true south
                    ...
                    dz = np.asarray(info_osp2.iloc[0,:])[0]-np.asarray(info_osp2.iloc[0,:])[2]
...
# =============================================================================
                    ...
                    [Cux[flag_case,0],Cuy[flag_case,0]],  pcov = curve_fit(Coh_lateral,f_s[indfu_s[0]:indfu_e[0],0], Coh_u_s[indfu_s[0]:indfu_e[0],0],p0=(0.5,6),maxfev=100000000)
                    [Cvx[flag_case,0],Cvy[flag_case,0]],  pcov = curve_fit(Coh_lateral,f_s[indfu_s[0]:indfu_e[0],0], Coh_v_s[indfu_s[0]:indfu_e[0],0],p0=(0.5,6),maxfev=100000000)
                    [Cwx[flag_case,0],Cwy[flag_case,0]],  pcov = curve_fit(Coh_lateral,f_s[indfu_s[0]:indfu_e[0],0], Coh_w_s[indfu_s[0]:indfu_e[0],0],p0=(0.5,6),maxfev=100000000)
                    ...
                    Cuz[flag_case,0],  pcov = curve_fit(Coh_vertical,f_s[indfu_s[0]:indfu_e[0],0], Coh_uz_s[indfu_s[0]:indfu_e[0],0],p0=(6),maxfev=100000000)
                    Cvz[flag_case,0],  pcov = curve_fit(Coh_vertical,f_s[indfu_s[0]:indfu_e[0],0], Coh_vz_s[indfu_s[0]:indfu_e[0],0],p0=(6),maxfev=100000000)
                    Cwz[flag_case,0],  pcov = curve_fit(Coh_vertical,f_s[indfu_s[0]:indfu_e[0],0], Coh_wz_s[indfu_s[0]:indfu_e[0],0],p0=(6),maxfev=100000000)
                ...
                #%%
                    ...
                    fig.suptitle('Storm No.' + str(event+1)+ ' Dir=' + str(np.round(meanDir[0,0],1)) + '$^o$'+'  Osp2 at height ' + str(np.round(np.asarray(info_osp2.iloc[0,:])[0],1)) + 'm ' \
                    ...
                    ax1.semilogx(f,Coh_lateral(f,Cvx[flag_case,0],Cvy[flag_case,0]),  'r-', label='$Fitted$ (Cvx='+ str(np.round(Cvx[flag_case,0],2))+';Cvy='+str(np.round(Cvy[flag_case,0],2))+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.semilogx(f,Coh_lateral(f,6,6.5),  'b--', label='$Ref$ (Cvx='+ str(3)+';Cvy='+str(10)+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.fill_between(f,-10,10,where= f>f_hor,facecolor='grey', alpha=0.5)   
                    ax1.set_xlim(0.005, 2)
                    ...
                    ax1.semilogx(f,Coh_lateral(f,Cwx[flag_case,0],Cwy[flag_case,0]),  'r-', label='$Fitted$ (Cwx='+ str(np.round(Cwx[flag_case,0],2))+';Cwy='+str(np.round(Cwy[flag_case,0],2))+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.semilogx(f,Coh_lateral(f,3,6.5),  'b--', label='$Ref$ (Cwx='+ str(3)+';Cwy='+str(10)+')' \
                                 ,linewidth=1, markeredgecolor='k',markersize=6)
                    ax1.fill_between(f,-10,10,where= f>f_hor,facecolor='grey', alpha=0.5)   
                    ax1.set_xlim(0.005, 2)
                    ...
                    path      = 'E:/DATA/Plots/Osp2 Coherence fitting/'
                    save_tite = 'Storm No.' + str(event+1) + '_coherence fitting Osp2_'\
