...
# =============================================================================
    ...
    # get all time tags in one cluster, its index in Osp1, and U, get the time tag with largesst U
    ...
    for ii in range(0,ttt.size):
        ttt_d[ii]   = mdates.date2num(mdates.num2date(ttt[ii]).replace(microsecond=0, second=0, minute=30)) 
    ...
    time_ss_ref.append(mdates.num2date(ttt_d[idx_Uttt]).replace(microsecond=0, second=0, minute=00) )
...
ax2.xaxis.set_major_locator(locator)
ax2.xaxis.set_major_formatter(formatter)
ax2.plot(mdates.num2date(Osp2['Time']), Osp2['B_U'], 'k-',label=r'$\overline{U} $ $(Osp2\_A)$',markeredgecolor='k',markersize=8)
ax2.plot(Time_Osp2_A_storm, Osp2['B_U'][Osp2_A_p],  'r*',label='Storm (Osp2)',markeredgecolor='r',markersize=12)
datemin = np.datetime64(time_s, 'm')
datemax = np.datetime64(time_e, 'm') + np.timedelta64(1, 'm')
...
fig.tight_layout(rect=[0, 0, 1, 0.95])
save_tite = 'Comparison of 1-h mean wind velocities among top sensors and Slåtterøy Fyr.png'
fig.savefig(save_tite) 
